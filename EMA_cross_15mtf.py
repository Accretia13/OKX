...
parser.add_argument("--telegram_token", default="7621973164:AAGjM-yVWPu5058DE8F6X_tcsEmyMV39V7o", help="Telegram Bot Token")
...
def calculate_hma(data, period):
    """
    Вычисляет Hull Moving Average (HMA) для заданного периода.
    """
...
async def fetch_tickers(threshold=1000000):
    """
...
async def fetch_historical_candles(ticker, limit=HISTORICAL_LIMIT):
    """
    Получает исторические свечи для тикера с REST API OKX и рассчитывает индикаторы.
    """
    url = "https://www.okx.com/api/v5/market/candles"
    params = {"instId": ticker, "bar": "15m", "limit": str(limit)}
...
# ------------------------- Проверка сигналов пересечения EMA -------------------------
def check_ema_cross(df):
    """
    Проверяет последние 8 свечей на наличие пересечения EMA(9) и EMA(21).
    Возвращает 'long' или 'short' при обнаружении пересечения.
    """
    recent = df.tail(8)
    diff = recent["ema_9"] - recent["ema_21"]
    prev = None
    signal = None
    for value in diff:
        if prev is not None:
...
async def check_initial_signals(tickers, chat_id):
    """
    Проверяет для каждого тикера, был ли пересечение EMA за последние 2 часа.
...
def generate_chart(ticker):
    try:
        ...
        # Удаляем строки, где отсутствуют индикаторы (если они не пересчитаны)
        df = df.dropna(subset=["ema_9", "ema_21", "hma_50"])
        numeric_cols = ["open", "high", "low", "close", "volume", "ema_9", "ema_21", "hma_50"]
        for col in numeric_cols:
            df[col] = pd.to_numeric(df[col], errors="coerce").ffill().fillna(0)
        add_plots = [
            mpf.make_addplot(df["ema_9"], color="blue", width=1.5),
            mpf.make_addplot(df["ema_21"], color="red", width=1.5),
            mpf.make_addplot(df["hma_50"], color="green", width=1.5)
        ]
        buf = io.BytesIO()
        fig, ax = mpf.plot(
            df,
...
async def send_waiting_message(chat_id):
    ...
    чтобы тот мог запустить повторный запрос данных.
    """
    keyboard = {
        "inline_keyboard": [
