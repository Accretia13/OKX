...
def get_last_saved_ts(file_name):
    ...
    with open(file_name, "r", encoding="utf-8") as f:
        lines = f.readlines()
    # Ищем последнюю строку с данными (не шапку)
    for line in reversed(lines):
        if line and not line.startswith("<"):
            parts = line.strip().split(",")
            if len(parts) >= 4:
                # дата + время в формате ГГГГММДД + ЧЧММСС
                date, time = parts[2], parts[3]
                dt = datetime.strptime(date + time, "%Y%m%d%H%M%S")
                # Преобразуем в unix ms
...
def fetch_and_save(inst_id: str, tf="3m", limit=100, total_candles=400000):
    ...
    file_name = f"{ticker}_{tf}_{total_candles}.txt"

    # Resume: если файл уже есть — ищем последнюю метку времени (ts)
    after = get_last_saved_ts(file_name)
    loaded_candles = 0
    if after:
        print(f"🔄 [{inst_id}] Resume mode: found file, last after={after}")
        # Подгружаем существующие свечи в candles, чтобы не дублировать
        with open(file_name, "r", encoding="utf-8") as f:
            for line in f:
                if line and not line.startswith("<"):
                    parts = line.strip().split(",")
        ...
        while len(candles) < total_candles:
            ...
            while attempt < max_attempts:
                try:
                    ...
                    break  # если нет ошибки — выходим из цикла попыток
            ...
            # Обработка и вставка в candles
            for c in data:
                ts = int(c[0])
                if ts not in candles:  # Не дублируем свечи
                    candles[ts] = {
                        "t": from_str_ms(c[0]),
                        "o": float(c[1]),
                        "h": float(c[2]),
    ...
    # Сохраняем результат (шапку пишем только если файла не было)
...
# Пример вызова для всех тикеров:
inst_ids = [
    "BTC-USDT-SWAP",
    "ETH-USDT-SWAP",
    "XRP-USDT-SWAP",
